...
describe('runProcessCommand', () => {
  it('captures stdout/stderr and supports stdin input', async () => {
    const script = [
    ...
    const result = await runProcessCommand({
      ...
      cwd: process.cwd(),
      env: process.env,
      inputText: 'hello-aider\n',
      timeoutMs: 5_000,
    });

    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('OUT:hello-aider');
    expect(result.stderr).toContain('ERR:warn');
    expect(result.timedOut).toBe(false);
  });

  it('terminates timed out process', async () => {
    const result = await runProcessCommand({
      command: process.execPath,
  ...
  it('sends SIGKILL to process that ignores SIGTERM after timeout', async () => {
    // Script that traps SIGTERM and refuses to die, forcing the SIGKILL fallback
    const script = [
      "process.on('SIGTERM', () => {});",
      'setTimeout(() => {}, 30_000);',
    ].join('\n');
